...
using namespace std;
    ...
    {
        return atan2(0.5* (sin(l)+sin(r)), 0.5*(cos(l)+cos(r)));
    }


class controlMocapNet
{
private:
    /// ROS nodehanlder
    ros::NodeHandle nh;
    ...
    VectorXd jointNominalConfig, jointNominalVelocity, qd, dqd, jointMocapNominalConfig;
    std::string com_topic, odom_topic, joint_state_topic, joint_cmd_topic, zmp_topic, wrenchRLeg_topic, wrenchLLeg_topic, LLegodom_topic, RLegodom_topic, gait_phase_topic;
...
public:
    ...
    ros::Subscriber odom_sub, zmp_sub, com_sub, joint_state_sub, joint_cmd_sub, wrenchLLeg_sub, wrenchRLeg_sub, LLegodom_sub,  RLegodom_sub, gait_phase_sub;
    ~controlMocapNet();
    controlMocapNet(ros::NodeHandle nh_);
    void run();
    ...
    void jointCmdCb(const sensor_msgs::JointStateConstPtr &msg);
};
#endif
