...
#include <lipm_control/FootPolygon.h>
        ...
        void FootPolygon::setPolygon(Vector3d footA_, Affine3d T)
        {
            ...
            footLF =   T.linear() * Vector3d(stepXF,stepYL,0.0) + footA;
            footLH =   T.linear() * Vector3d(stepXH,stepYL,0.0) + footA;
            footRF =   T.linear() * Vector3d(stepXF,stepYR,0.0) + footA;
            footRH =   T.linear() * Vector3d(stepXH,stepYR,0.0) + footA;
        ...
        bool FootPolygon::checkIn(double x_, double y_)
        {
        ...
        for(int k = 0; k<numNodes; k++)
        {
            ...
            if(k==numNodes-1)
            {
                current_poly = Vector3d(polygon(0,0) - polygon(k,0), polygon(0,1) - polygon(k,1), 0.0);
            }
            else
            {
                current_poly = Vector3d(polygon(k+1,0) - polygon(k,0), polygon(k+1,1) - polygon(k,1), 0.0);
            }

            //vector from point in space vertex on polygon (point-p(k))
            Vector3d point_vect = Vector3d(x_ - polygon(k,0), y_ - polygon(k,1), 0.0);
            ...
            Vector3d c = current_poly.cross(point_vect);
            if(c(2)>0)
            {
                current_sign = 1;
            }
        ...
        bool FootPolygon::pnpoly(double x, double y)
        {
